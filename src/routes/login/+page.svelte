<script lang="ts">
	...
	import { ArrowLeft, Mail, Lock, LoaderCircle, CircleCheck, TriangleAlert } from "lucide-svelte";
...
<div
	class="fixed inset-0 bg-background flex flex-col items-center justify-center p-6 overflow-hidden"
	style="padding-top: env(safe-area-inset-top); padding-bottom: env(safe-area-inset-bottom); padding-left: max(1.5rem, env(safe-area-inset-left)); padding-right: max(1.5rem, env(safe-area-inset-right));"
...
>
	<div class="w-full max-w-md space-y-8">
		...
		>
			...
			{#if resetMessage}
				<div
					class="rounded-2xl border border-green-500/20 bg-green-500/10 px-4 py-3 text-green-700 dark:text-green-300"
					in:fly={{ y: 8, duration: 220 }}
					out:fade={{ duration: 180 }}
				>
					<div class="flex items-start gap-3">
						<CircleCheck size={18} class="mt-0.5 flex-shrink-0" />
						<p class="text-sm">{resetMessage}</p>
					</div>
				</div>
			{/if}
